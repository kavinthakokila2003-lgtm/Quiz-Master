const initial = () => ({
  ...
  points:10, speedBonus:0, qcount:0, passwords:{}, answers:[], questions:[], projectedQuestionId:null, phase:'admin', tab:'Overview'
});
...
function safeState(s){const done=isComplete(s),qs=normalizedQuestions(s),projected=qs.find(q=>q.id===String(s.projectedQuestionId));return {quizName:s.quizName,running:s.running,leaderboard:s.leaderboard,rounds:(s.rounds||[]).map(({name,q,time,status,n,active,ended,deadlineAt})=>({name,q,time,status,n,active,ended,deadlineAt})),teams:(s.teams||[]).map(({name,points,progress,status,logo,logoColor})=>({name,points,progress,status,logo,logoColor})),allTeamsComplete:done,projectedQuestion:projected?{id:projected.id,round:projected.round,text:projected.text,options:projected.options,answer:done?projected.answer:null,points:projected.points}:null,finalQuestions:done?qs.map(({id,round,text,options,answer,points})=>({id,round,text,options,answer,points})):[],updatedAt:Date.now()};}
...
async function handleApi(req,env,url){
 ...
 if(m==='PUT'&&p==='/api/admin/state'){
   ...
   await changeState(env,old=>{for(const team of s.teams){const saved=old.teams.find(t=>t.code===team.code);if(saved){team.points=Number(saved.points)||0;team.speed=Number(saved.speed)||0;team.progress=saved.progress;team.status=saved.status}}s.answers=old.answers||[];Object.assign(old,s);return null});return json({ok:true,updatedAt:Date.now()});
 }
 if(m==='POST'&&p==='/api/team/verify-round'){
   const ses=await token(req,env,'team');if(!ses)return fail(401,'Enter your team login code first');const b=await body(req),{state}=await getState(env),n=Number(b?.round),r=(state.rounds||[]).find(x=>x.n===n),team=(state.teams||[]).find(t=>t.code===ses.code),expected=state.passwords?.[`${n}-${ses.code}`],progress=Number((team?.progress||'0/0').split('/')[0])||0;if(expireRound(state,r)){await syncExpired(env);return fail(409,'This round is closed')}if(!state.running||!r||!r.active)return fail(409,'This round is not open');if(!team||progress!==n-1)return fail(409,'Finish the previous round before starting this one');if(n>1&&(!expected||String(b.password||'').trim().toUpperCase()!==expected))return fail(401,'Incorrect round password');const questions=normalizedQuestions(state).filter(q=>q.round===n);return json({ok:true,round:{name:r.name,n:r.n,questions:questions.length,time:r.time,deadlineAt:r.deadlineAt},questions:questions.map(({id,text,options,points})=>({id,text,options,points}))});
 }
 if(m==='GET'&&p==='/api/team/state'){
   const ses=await token(req,env,'team');if(!ses)return fail(401,'Team login required');await syncExpired(env);const {state:s}=await getState(env),t=s.teams.find(x=>x.code===ses.code),rows=(s.answers||[]).filter(a=>a.team_code===ses.code).sort((a,b)=>a.round_no-b.round_no||a.question_id.localeCompare(b.question_id)),reveal=Number((t?.progress||'0/0').split('/')[0])>=s.rounds.length;return json({team:t?{name:t.name,code:t.code,points:t.points,progress:t.progress,status:t.status,logo:t.logo,logoColor:t.logoColor}:null,state:safeState(s),answers:rows.map(({correct_answer,...a})=>({...a,correct:reveal?a.correct:null,...(reveal?{correct_answer}: {})})),revealAnswers:reveal});
 }
